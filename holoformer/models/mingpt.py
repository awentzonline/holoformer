...
class MinGPT(pl.LightningModule):
    ...
    def __init__(self, tokenizer, data_dims=100, ff_dims=512, layers=4,
        ...
        self.gpt_config = GPTConfig(
            len(tokenizer), max_seq_len, n_layer=layers,
            n_head=heads, n_embd=data_dims, weight_decay=weight_decay,
            learning_rate=lr, betas=opt_betas,
        )
    ...
    def _shared_step(self, data, batch_idx):
        ...
        target_tokens = all_tokens[:, 1:].contiguous()
        p_tokens, loss = self.model(all_tokens[:, :-1], target_tokens)
        # p_tokens = p_tokens[:, :-1]
    ...
    def configure_optimizers(self):
        return self.model.configure_optimizers(self.gpt_config)
