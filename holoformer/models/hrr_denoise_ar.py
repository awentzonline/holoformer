...
class DenoiseHRRAR(pl.LightningModule):
    ...
    def __init__(self, tokenizer, data_dims=100, ff_dims=512, layers=4,
        ...
        self.noise_predictor = nn.Sequential(
            nn.Linear(2 * data_dims, data_dims),
            nn.GELU(),
            #nn.BatchNorm1d(data_dims),
    ...
    def forward(self, x, hidden=None, **kwargs):
        ...
        all_embs = self.embedding.weight.sum(0)
        all_embs = all_embs[None, None, ...].repeat((batch_size, seq_len, 1))
        input_embs = self.embedding(x)
        #print(input_embs.shape, all_embs.shape)
        denoised_embs = self.denoise_tokens(input_embs, all_embs, steps=self.steps)
        # encoded, hidden = self.encode_sequence(x, hidden)
        # encoded = encoded / (torch.norm(encoded, dim=-1, keepdim=True) + 1e-8)
        return self.output_token(denoised_embs)
    ...
    def _shared_step(self, data, batch_idx):
        input_tokens = data['input_ids'].clone()
        batch_size, seq_len = input_tokens.shape
        all_embs = self.embedding.weight.sum(0)
        all_embs = all_embs[None, None, ...]#.repeat((batch_size, seq_len, 1))
        input_embs = self.embedding(input_tokens)
        target_embs = input_embs[:, 1:]
        noised_embs = self.add_noise_to_tokens(target_embs, all_embs)
        target_noise = noised_embs - target_embs
        state = torch.cumsum(input_embs[:, :-1], dim=1)
        p_noise = self.noise_predictor(torch.cat([
            state, noised_embs
        ], dim=-1))

        noise_recon_loss = F.mse_loss(p_noise, target_noise)

        # p_tokens, _ = self(input_tokens[:, :-1])
        ...
        loss = noise_recon_loss
        metrics = dict(
            loss=loss,
            noise_recon_loss=noise_recon_loss,
        )
        losses = dict(
            loss=loss
        )
    ...
    def add_noise_to_tokens(self, tokens, noise):
        delta = np.random.random()
        #noised_tokens = delta * tokens + (1 - delta) * noise
        noised_tokens = tokens + delta * noise
        return noised_tokens

    def denoise_tokens(self, states, noised_embs, steps=1):
        state = torch.cumsum(states, axis=1)
        for i in range(steps):
            p_noise = self.noise_predictor(torch.cat([
                state, noised_embs
            ], dim=-1))
            noised_embs = noised_embs - p_noise
        return noised_embs
    ...
    def configure_optimizers(self):
        ...
        blacklist_weight_modules = (
            torch.nn.LayerNorm, torch.nn.Embedding, torch.nn.LSTM, #Dot,
            # nn.BatchNorm2d
        )
...
if __name__ == '__main__':
    ...
    p = DenoiseHRRAR.add_argparse_args(p)
