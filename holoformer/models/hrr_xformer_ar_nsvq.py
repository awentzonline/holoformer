...
from .hrr_nsvq import HRRNSVQMultiLabel
...
class HolographicQKV(nn.Module):
    def __init__(self, dims, heads, causal=True, unit=True):
        ...
        self.op_codebook = HRRNSVQMultiLabel(512, dims)
...
class LearnDot(nn.Module):
    def __init__(self, in_dims, out_dims, normalize=True):
        super().__init__()
        self.weight = nn.Parameter(
            torch.zeros(out_dims, in_dims)
        )
        nn.init.xavier_uniform(
            self.weight, gain=nn.init.calculate_gain('sigmoid')
        )
...
class DotLoss(nn.Module):
    def forward(self, pred, target):
        dense_targets = torch.zeros_like(pred)
        dense_targets[torch.arange(len(pred)), target] = 1.
        weights = dense_targets + (1 - dense_targets) * (len(target) / pred.numel())
        dense_targets = dense_targets * 2 - 1  # make it [-1, 1]
        return ((1 - (dense_targets * pred))).sum()
        #return ((1 - (dense_targets * pred)) * weights).sum()
...
class HoloformerAR(pl.LightningModule):
    ...
    def __init__(self, tokenizer, data_dims=100, ff_dims=512, layers=4,
        ...
        #     data_dims, nhead=heads, dim_feedforward=ff_dims, dropout=dropout,
        #     batch_first=True,
    ...
    def get_output_head(self, output_dims):
        return nn.Sequential(
            ...
            LearnDot(self.data_dims, output_dims),
        )
    ...
    def forward(self, x, **kwargs):
        ...
        encoded_present = hrr.unbind(encoded, present_emb)
        return self.output_token(encoded_present)
    ...
    def outputs_to_ids(self, x, temperature=0., top_p=0.95, top_k=0):
        if temperature:
            x = x / temperature
        if top_p or top_k:
    ...
    def generate(self, prompt, max_length=None, temperature=0.0, top_p=0.95, top_k=0.):
    ...
    def configure_optimizers(self):
        ...
        decay = set()
        no_decay = set()
        whitelist_weight_modules = (torch.nn.Linear, nn.TransformerEncoderLayer, nn.TransformerEncoder, LearnDot)
    ...
    def add_argparse_args(self, p):
        ...
        p.add_argument('--ff_dims', default=2048, type=int)
